...
def invest():
    global drive_time, used_cups
    drive_time = int_how_long
    used_cups = how_much_entry

def types():
    if gas_type[0]:
        CO2_disel = 2.68
        global total_CO2
        total_CO2 = CO2_disel * drive_time
        return total_CO2

    else:
        CO2_petrol = 2.31

        total_CO2 = CO2_petrol * drive_time
        return total_CO2

def disposables(used_cups):
    global total_disposables
    total_disposables = 0.08 * used_cups
    return total_disposables
...
# create a new text file to store what users have input
td = disposables(used_cups)
tc = types()
f = open("data.txt", "w")
f.write(td, tc)
f.close()
...
resized = logo_image.resize((100, 80), Image.Resampling.LANCZOS)
